...
public class ConvertToAtomicLongRefactoring extends Refactoring {
	...
	public ConvertToAtomicLongRefactoring(IField field){
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm)
		...
		pm.beginTask("", 12);
		pm.setTaskName("Convert to AtomicLong checking preconditions");
		pm.worked(1);
		if (result.hasFatalError())
			return result;
		pm.setTaskName("ConvertToAtomicLong searching for cunits"); 
		...
		for (int i= 0; i < affectedCUs.length; i++) {
			...
			AccessAnalyzerForAtomicLong analyzer= new AccessAnalyzerForAtomicLong(this, unit, fieldIdentifier, declaringClass, rewriter, importRewrite);
	...
	private Collection addChangeDeclaringType(CompilationUnit root) {
		...
		Type newType= null;
		newType= root.getAST().newSimpleType(ASTNodeFactory.newName(root.getAST(), "AtomicLong"));
		...
		if (initializeDeclaration) {
			...
			atomicInstanceCreation.setType(root.getAST().newSimpleType(ASTNodeFactory.newName(root.getAST(), "AtomicLong")));
			if (initializer != null) {
	...
	private String getMappingErrorMessage() {
		return Messages.format(
			"Convert to AtomicLong cannot analyze selected field", 
	...
	public Change createChange(IProgressMonitor pm) throws CoreException,
		...
		String description = "Convert long to AtomicLong";
		String comment = "Convert long to AtomicLong";
		...
		pm.setTaskName("ConvertToAtomicLong create changes");
		...
		for (int i= 0; i < messages.length; i++) {
			...
			if (!isIgnorableProblem(problem)) {
				result.addError(Messages.format(
						"ConvertToAtomicLong: Compiler errors", 
